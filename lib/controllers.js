...
/**
 * Get the NodeBB API URL for this site (for multi-tenant support)
 * Derives from the configured base URL, e.g., https://nodebb.example.com/sitename -> https://nodebb.example.com/sitename/api
 */
function getNodeBBSiteUrl() {
...
/**
 * Get channel mappings from AWS (filtered by this NodeBB site)
 */
controllers.getMappings = async (req, res) => {
	try {
		// Get the NodeBB API URL for this site to filter mappings
		const nodebbSiteUrl = getNodeBBSiteUrl();
		const mappings = await awsWebhook.getMappings(nodebbSiteUrl);
...
controllers.saveMapping = async (req, res) => {
	try {
		const { channelId, channelName, categoryId, slackTeamId } = req.body;
		...
		// Get the NodeBB API URL for this site (critical for multi-tenant filtering)
		const nodebbSiteUrl = getNodeBBSiteUrl();
		...
		winston.info('[slack-bidirectional] Mapping saved:', { channelId, categoryId, nodebbSiteUrl });
