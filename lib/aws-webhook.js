 ...
 * Calls the AWS API to retrieve channel-category mappings filtered by this NodeBB site
 * @param {string} nodebbSiteUrl - The NodeBB API URL for this site (for multi-tenant filtering)
 */
awsWebhook.getMappings = async (nodebbSiteUrl = null) => {
	const settings = await getSettings();
	...
	// Derive the mappings endpoint from the webhook URL
	// Webhook: https://xxx.execute-api.region.amazonaws.com/dev/nodebb/events
	// Mappings: https://xxx.execute-api.region.amazonaws.com/dev/mappings
	const baseUrl = settings.awsWebhookUrl.replace(/\/nodebb\/events\/?$/, '');
	let mappingsUrl = `${baseUrl}/mappings`;

	// Add siteUrl query parameter for multi-tenant filtering
	if (nodebbSiteUrl) {
		mappingsUrl += `?siteUrl=${encodeURIComponent(nodebbSiteUrl)}`;
	}
	...
	if (settings.awsApiKey) {
		headers['x-api-key'] = settings.awsApiKey;
	}
	...
	const result = await response.json();
	// API wraps response in { success: true, data: { mappings: [...] } }
	return result.data?.mappings || result.mappings || [];
};

/**
 * Save a channel mapping via AWS
 * @param {Object} mapping - Mapping data
 * @param {string} mapping.channelId - Slack channel ID
 * @param {string} mapping.channelName - Slack channel name
 * @param {number} mapping.categoryId - NodeBB category ID
 * @param {string} mapping.categorySlug - NodeBB category slug
 * @param {string} mapping.nodebbSiteUrl - NodeBB API URL (for multi-tenant filtering)
 * @param {string} mapping.slackTeamId - Slack team/workspace ID
 */
awsWebhook.saveMapping = async ({ channelId, channelName, categoryId, categorySlug, nodebbSiteUrl, slackTeamId }) => {
	const settings = await getSettings();
...
/**
 * Delete a channel mapping via AWS
 */
awsWebhook.deleteMapping = async (channelId) => {
	const settings = await getSettings();
	...
	const baseUrl = settings.awsWebhookUrl.replace(/\/nodebb\/events\/?$/, '');
	const mappingsUrl = `${baseUrl}/mappings/${encodeURIComponent(channelId)}`;
